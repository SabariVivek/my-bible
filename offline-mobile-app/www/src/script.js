...
// Helper function to check if a verse is a memory verse (disabled)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        if (avatarEl) {
            ...
            // For guest users: ensure settings button is visible
            if (isGuest) {
    ...
    // Show right-menu-btn (the button in header-controls that opens Settings)
    if (rightMenuBtn) {
        rightMenuBtn.style.display = 'flex';
    }
...
// Verse Images Manager - Disabled
function getVerseImageFilename() { return ''; }
function getVerseImagePath() { return ''; }
function getVerseImageHTML() { return Promise.resolve(''); }
async function getVerseImagesForChapter() { return {}; }
function loadVerseImages() { }

// Generate king transition card HTML for a verse (disabled)
function getKingTransitionCardHTML() { return ''; }
...
function initializeSearch() {
    ...
    async function performSearch(query) {
        ...
        if (bibleDataManager.supabaseClient && navigator.onLine) {
            try {
        ...
        if (!loadedFromSupabase && results.length === 0) {
            try {
                const searchLanguage = isTamil ? 'tamil' : 'english';
                ...
                for (const book of booksToSearch) {
                    if (localRows.length >= 100) break;
                    const bookFile = getBookFileForLanguage(book.file, searchLanguage);
                    const bookData = await bibleDataManager.loadEntireBook(bookFile, searchLanguage);
                    if (!bookData) continue;

                    const chapterKeys = (selectedChapter !== '' && bookData[selectedChapter])
                        ? [selectedChapter]
                        : Object.keys(bookData);

                    for (const chapKey of chapterKeys) {
                        if (localRows.length >= 100) break;
                        const chapterVerses = bookData[chapKey];
                        if (!chapterVerses) continue;

                        Object.keys(chapterVerses).forEach(vk => {
                            if (localRows.length >= 100) return;
                            const verseText = chapterVerses[vk];
                            if (typeof verseText === 'string' && verseText.toLowerCase().includes(queryLower)) {
                                const vNum = parseInt(vk.replace('verse_', ''));
                                localRows.push({
                                    book_file: book.file,
                                    chapter: parseInt(chapKey),
                                    verse: vNum,
                                    text: verseText
                                });
                            }
...
// Memory Verses - Disabled
async function loadMemoryVersesFromSupabase() { return true; }
async function toggleMemoryVerse() { return false; }
async function saveMemoryVersesToSupabase() { return true; }
async function cleanupDuplicateMemoryVerses() { return true; }
function markBooksWithMemoryVerses() { }
function updateVerseMemoryVerseIndicators() { }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Click rightMenuBtn in header-controls to directly open Settings panel
    if (rightMenuBtn) {
        rightMenuBtn.style.display = 'flex';
